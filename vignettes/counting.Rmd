...
By default, the counting functions will only search the read sequence on the original strand reported by the sequencer.
It is possible to change this behaviour with the `strand=` argument, to search the reverse strand instead or to search both strands.
...
- Single-end sequencing of a construct where the barcode can be covered by reads from either end.
  ...
  Note that users should make sure that the constant regions are captured by reads from both ends.
