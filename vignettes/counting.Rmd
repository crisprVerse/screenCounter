...
out <- matrixOfComboBarcodes(combo.fq,
    ...
    choices=list(first=known, second=known))    
out$combination
out$counts
```

This function produces a matrix of frequencies of each barcode (row) in each file (column).
The identities of the variable regions in each combinatorial barcode are specified in the `combination` data frame, which contains integer indices to the variable sequences in `known`.
