...
We assume we have 1000 barcodes that have assignments to 100 genes.
Of these, a small fraction are control genes that should not exhibit any change over time.
...
In this case, we combine $p$-values across barcodes using Simes' method, which tests the null hypothesis that all barcodes for a given gene are not differentially abundant.
...
Alternatively, we can switch to a different consolidation strategy with the `method` argument - see below. 
...
The `method` argument supports different consolidation strategies that favor detection of different genes:
