...
We combine $p$-values across barcodes using Simes' method, which tests the null hypothesis that all barcodes for a given gene are not differentially abundant.
Log-fold changes and log-counts-per-million are reported for the "best" barcode (i.e., with the lowest $p$-value) in each gene's pool.
