#' DataFrame for feature-level screen results
#'
...
#' 
#' @docType class
#' @name ScreenFeatureStatFrame
#' @aliases ScreenFeatureStatFrame ScreenFeatureStatFrame-class .trackCheck,ScreenFeatureStatFrame-method
#' .trackReqCols,ScreenFeatureStatFrame-method .trackRowIds,ScreenFeatureStatFrame-method
NULL

#' @export
#' @importFrom gp.sa.core .createTDFSubclass
ScreenFeatureStatFrame <- function(x, ...) {
    x <- .createTDFSubclass(x, ...)
    as(x, "ScreenFeatureStatFrame")
}

#' @export
#' @importFrom gp.sa.core .trackCheck trackinfo trackinfo<- .quickError
#' @importFrom S4Vectors isSingleString
setMethod(".trackCheck", "ScreenFeatureStatFrame", function(x) {
    trackinfo(x)$type <- "per-feature differential screen abundance"
    callNextMethod()
})

#' @export
#' @importFrom gp.sa.core .trackReqCols
setMethod(".trackReqCols", "ScreenFeatureStatFrame", function(x) {
    out <- callNextMethod()              
    out[[1]] <- c(out[[1]], "NBarcodes")
    out[[2]] <- c(out[[2]], "integer")
    out
})
