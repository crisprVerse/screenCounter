#' DataFrame for barcode-level screen results
#'
...
#' 
#' @docType class
#' @name ScreenBarcodeStatFrame
#' @aliases ScreenBarcodeStatFrame ScreenBarcodeStatFrame-class .trackCheck,ScreenBarcodeStatFrame-method
#' .trackRowIds,ScreenBarcodeStatFrame-method
NULL

#' @export
#' @importFrom gp.sa.core .createTDFSubclass
ScreenBarcodeStatFrame <- function(x, ...) {
    x <- .createTDFSubclass(x, ...)
    as(x, "ScreenBarcodeStatFrame")
}

#' @export
#' @importFrom gp.sa.core .trackCheck trackinfo trackinfo<- .quickError
#' @importFrom S4Vectors isSingleString
setMethod(".trackCheck", "ScreenBarcodeStatFrame", function(x) {
    trackinfo(x)$type <- "per-barcode differential screen abundance"
    callNextMethod()
})
